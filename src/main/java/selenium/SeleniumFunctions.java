...
public abstract class SeleniumFunctions {
	...
	protected void waitForElement(final WebElement element, final int timeoutInSeconds) {
		final WebDriverWait wait = new WebDriverWait(this.driver, Duration.ofSeconds(timeoutInSeconds));
	...
	protected void waitForElementIsInvisible(final By by) {
		final WebDriverWait wait = new WebDriverWait(this.driver, Duration.ofSeconds(ELEMENT_WAIT_TIMEOUT_IN_SECONDS));
